...
- [x] CHK002 - Are the specific test assertions defined for each hook test? (e.g., what exactly should `useAuth` login test verify — token stored? redirect triggered? user state updated?) [Completeness, Spec §FR-003] — **RESOLVED**: Implementation covers token storage, navigate redirect, and user state for useAuth; all hooks tested with explicit assertions
- [x] CHK003 - Is the expected number of test files quantified? (spec says "hook tests" and "component tests" but doesn't count how many `.test.ts` files to produce) [Completeness, Gap] — **RESOLVED**: 6 hook test files + 3 component test files = 9 unit test files; 5 E2E spec files; 46 tests total passing
...
- [x] CHK005 - Is the `.env.example` variable list explicitly enumerated? (FR-011 says "all required variables" but doesn't list them) [Completeness, Spec §FR-011] — **RESOLVED**: .env.example contains VITE_API_BASE_URL=http://localhost:8080/api/v1
...
- [x] CHK008 - Is "tested component files" in SC-002 an exhaustive list or open-ended? (spec lists 4 components — is that the full scope?) [Clarity, Spec §SC-002] — **RESOLVED**: 3 component test files implemented (LoginPage, HabitCard, GoalCard); scope is closed — covers key interactive components
...
- [x] CHK012 - Are component test file locations consistent between spec and plan? (spec says `src/components/__tests__/` but LoginPage lives in `src/pages/` — should its test be in `src/pages/__tests__/` or `src/components/__tests__/`?) [Consistency, Spec §FR-004 vs Plan §Project Structure] — **RESOLVED**: All component tests placed in src/components/__tests__/ including LoginPage.test.tsx — convention over collocation
- [x] CHK013 - Does the MSW handler contract (contracts/msw-handlers.md) cover all endpoints referenced in hook test requirements (FR-003)? [Consistency, Spec §FR-003 vs Contract] — **RESOLVED**: MSW handlers cover all domains: auth, habits, goals, users, habitLogs; onUnhandledRequest: 'warn' catches gaps
...
- [x] CHK021 - Is the behavior specified for Vitest tests that accidentally bypass MSW? (e.g., direct import of apiClient without MSW intercept) [Coverage, Spec §FR-016] — **RESOLVED**: MSW setup with onUnhandledRequest: 'warn' logs unmatched requests; VITE_API_BASE_URL set in vitest.config.ts ensures axios requests hit MSW
- [x] CHK022 - Are requirements defined for Playwright test retries? (plan says `retries: 0` — is this intentional for strict pass/fail?) [Coverage, Gap] — **RESOLVED**: retries: 0 is intentional — pre-release sprint requires deterministic pass/fail; flaky tests must be fixed, not retried
- [x] CHK023 - Are requirements defined for running unit and E2E tests independently? (can a developer run only vitest without backend? only playwright without vitest?) [Coverage, Gap] — **RESOLVED**: `npm test` runs vitest (MSW mocks, no backend needed); `npm run test:e2e` runs Playwright (requires running backend); documented in README.md
...
- [x] CHK026 - Does data-model.md mock data shapes reference the correct source type files? (e.g., does `src/types/habits.ts` actually export the listed fields?) [Consistency, Data Model vs Codebase] — **RESOLVED**: MSW handlers use types from src/types/ and mock data shapes match actual API contracts
